...
WHERE license_plate IN ("L93JTIZ", "322W7JE", "0NTHK55");
...
-- Check whether the thief went through which flight.
SELECT flight_id FROM passengers
WHERE passport_number IN (SELECT passport_number FROM people WHERE name = "Russell");

-- Name of city for the above flight id.
SELECT city FROM airports
WHERE id IN (SELECT destination_airport_id FROM flights WHERE id = 18);